...
/* 
1. As i can see in the given query are specify the SELECT and not using the (*) are agood efficiency where query not looking into the whole table but only the table that we needed only.
2. Using Left Join for combining can minimize the retrieval time of the query.
3. Already implement the pagination queries.

In order to improve performance more on the query:
1. First using the 'EXPLAIN' statement to analyze the query.
2. Avoid using wildcard (%). Example like JobTypes.name LIKE '%キャビンアテンダント%'. 
*/
